...
export async function submitServiceReport(ticketId, reportData) {
    return apiFetch(`/portal/tickets/${ticketId}/service-report`, {
        ...
        body: JSON.stringify({
            ...
            parts_used: (reportData.partsUsed || []).map(p => ({
                partId: p.partId,
                name: p.name,
                quantity: p.quantity || 1,
                unitCost: p.unitCost != null ? p.unitCost : (p.cost || 0),
                cost: p.cost || 0,
            })),
            total_parts_cost: reportData.totalPartsCost,
            is_warranty: reportData.isWarranty || false,
    ...
    return {
        quotation: data.quotation
            ? {
                ...
                isWarranty: data.quotation.is_warranty || false,
        ...
        serviceReport: data.service_report
            ? {
                ...
                partsUsed: (data.service_report.parts_used || []).map(p => ({
                ...
                isWarranty: data.service_report.is_warranty || false,
